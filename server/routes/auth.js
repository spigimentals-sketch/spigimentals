...
authRouter.post('/signup', async (req, res) => {
  ...
  db.prepare('INSERT INTO profiles (id, email, name) VALUES (?, ?, ?)').run(id, normalizedEmail, displayName);

  const token = signToken(id);
  res.status(201).json({ token, user: { id, email: normalizedEmail }, profile: getProfile(id) });
});
...
authRouter.post('/signin', async (req, res) => {
  ...
  res.json({ token, user: { id: user.id, email: user.email }, profile: getProfile(user.id) });
});
...
authRouter.get('/session', requireAuth, (req, res) => {
  ...
  res.json({ user, profile: getProfile(req.userId) });
});
