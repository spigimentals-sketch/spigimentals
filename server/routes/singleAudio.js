...
export function createSingleFileRouter(table, column, folder, uploadMiddleware, fieldLabel, resourceType) {
  ...
  router.post('/', requireAuth, requireAdmin, handleUpload(uploadMiddleware), async (req, res) => {
    if (!req.file) return res.status(400).json({ error: `No ${fieldLabel} file provided.` });

    const row = db.prepare(`SELECT ${column} FROM ${table} WHERE id = ?`).get(req.params.id);
    ...
    db.prepare(`UPDATE ${table} SET ${column} = ? WHERE id = ?`).run(result.secure_url, req.params.id);
  ...
  router.delete('/', requireAuth, requireAdmin, async (req, res) => {
    ...
    db.prepare(`UPDATE ${table} SET ${column} = NULL WHERE id = ?`).run(req.params.id);
    res.status(204).end();
  });
